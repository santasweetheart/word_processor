...
; num-divs : Div -> PosInt
; takes in a div and returns the number of divs inside of it 

(check-expect (num-divs div-1) 1)
(check-expect (num-divs div-2) 1)
(check-expect (num-divs ul-1) 3)
(check-expect (num-divs ol-1) 3)

(define (num-divs d)
  (cond [(string? d) 1]
        [(image?  d) 1]
        [(ul?     d) (add1 (length (ul-content d)))]
        [(ol?     d) (add1 (length (ol-content d)))]))
...
(define im-3 (triangle 10 "solid" "red"))

; enumerate : [List-of Image] -> [List-of Image]
; produces an enumerated list of images with the 

(check-expect (enumerate (list im-1 im-2))
              (list (beside (text "1. " TEXT-SIZE TEXT-COLOR) im-1)
                    (beside (text "2. " TEXT-SIZE TEXT-COLOR) im-2)))
(check-expect (enumerate (list im-1 im-2 im-3))
              (list (beside (text "1. " TEXT-SIZE TEXT-COLOR) im-1)
                    (beside (text "2. " TEXT-SIZE TEXT-COLOR) im-2)
                    (beside (text "3. " TEXT-SIZE TEXT-COLOR) im-3)))
(check-expect (enumerate '()) '())

(define (enumerate loi)
  (foldr (lambda (num im base) (cons (beside num im) base)) '() (build-loi loi) loi))
;(foldr proc init lst ...+), according to the documentation foldr is capable of taking more
;than one list. 


;build-loi: [List-of Image] -> [List-of Image]
;creates a list of of images where the each image represents the position (starting from 1) of each
;element in given list
(check-expect (build-loi (list im-1 im-2))
              (list (text "1. " TEXT-SIZE TEXT-COLOR) (text "2. " TEXT-SIZE TEXT-COLOR)))
(check-expect (build-loi (list im-1 im-2 im-3))
              (list (text "1. " TEXT-SIZE TEXT-COLOR) (text "2. " TEXT-SIZE TEXT-COLOR)
                    (text "3. " TEXT-SIZE TEXT-COLOR)))
(check-expect (build-loi empty) empty)

(define (build-loi loi)
  ;number->image: Number -> Image
  ;Turns the given number into text
  ; x= 1, (number->image 1) = (text "1. " TEXT-SIZE TEXT-COLOR)
  (local [(define (number->image x)
            (text (string-append (number->string (add1 x)) ". ") TEXT-SIZE TEXT-COLOR))]
    (build-list (length loi) number->image)))
